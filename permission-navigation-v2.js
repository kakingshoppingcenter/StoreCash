...
(function installPermissionNavigationV2() {
  if (window.__KSC_PERMISSION_NAVIGATION_V2__) return;
  window.__KSC_PERMISSION_NAVIGATION_V2__ = true;
  ...
  function installPendingStyle() {
    ...
    style.textContent = `
      html body[data-ksc-permissions-ready="false"] #appShell:not(.hidden):not([hidden]){
  ...
  function selectLandingView() {
    ...
    // Preserve valid deep links and previous modules, but do not allow a stale
    // automatic Dashboard selection to override the role's primary workflow.
  ...
  function ensureAuthorizedLanding() {
    ...
    // A deliberate click in this browser session always wins. Otherwise, correct
    // the legacy automatic Dashboard selection for Store Users and Checkers.
  ...
  async function refreshOwnProfile(force = false) {
    ...
    if (!force && activeUserId === userId && authorizationReady) return true;
    ...
    try {
      ...
      lastProfileSignature = profileSignature(currentProfile());
  ...
  const observer = new MutationObserver(() => {
    if (authorizationReady) queueApply();
  });
  observer.observe(document.documentElement, {
    subtree: true,
    childList: true,
    attributes: true,
    attributeFilter: ['class', 'hidden', 'data-view']
  });
  ...
  window.setInterval(async () => {
    const before = lastProfileSignature;
    await refreshOwnProfile(true);
    const after = profileSignature(currentProfile());
    if (after && after !== before) queueApply();
  }, PROFILE_REFRESH_MS);
